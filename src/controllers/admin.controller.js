...
const createDepartment=async(req,res)=>{
    ...
    res.status(200).render('admin/createDepartment',{success:"Department Created Successfully! 🎊"})
...
const viewDepartments=async(req,res)=>{
    try {
        ...
        const departments=await Department.find(query).skip(skip).limit(limit).sort({name:1})
...
const updateDepartment=async(req,res)=>{
    ...
    const {name,type,address}=req.body
    const updatedDepartment=await Department.findByIdAndUpdate(id,{name,type,address},{new:true})
    if (!updatedDepartment) {
      return res.status(404).render('admin/editDepartment', { error: "Department not found!" });
    }
    res.render('admin/editDepartment',{department:updatedDepartment,success:"Department Updated Successfully! 🎊"})
}
...
const createUser=async(req,res)=>{
    ...
    res.render('admin/createUser',{departments,success:"User created successfully! 🎊"})
}
...
const updateUser=async(req,res)=>{
    try {
        ...
        await user.save();
        const departments=await Department.find()
        res.render('admin/editUser',{user,departments,success:"User updated successfully! 🎊"})
    } catch (error) {
        console.log(error)
